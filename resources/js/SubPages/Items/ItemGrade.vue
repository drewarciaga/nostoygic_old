...
const { grade, grades, totalGrades, columns, errors,
        getGrade, getAllGrades, storeGrade, updateGrade, deleteGrade, resetFields
      } = useGrade()
...
async function deleteModel(){
    changeAction('Delete')
    isLoadingView.value = true
    await deleteGrade(model_id.value)
    model_id.value = ''
    deletemodelRef.value.hideConfirmDialog()
...
async function searchTable(search_input){
    if(search_input.value.toString().trim != ''){
        await getAllGrades(search_input.value)
    }else{
        await getAllGrades()
    }
}
...
<template>
    ...
    <div class="flex flex-wrap -mx-1 overflow-hidden sm:-mx-1 md:-mx-1 lg:-mx-1 xl:-mx-1">
        ...
        <div class="my-1 px-1 w-full overflow-hidden sm:my-1 sm:px-1 sm:w-1/2 md:my-1 md:px-1 md:w-1/2 lg:my-1 lg:px-1 lg:w-1/2 xl:my-1 xl:px-1 xl:w-1/2">
            <BreezeButton :type="'button'" data-bs-toggle="modal" data-bs-target="#gradeAddEditModal" @click="changeAction('Add')">
                <span class="mdi mdi-plus-circle"> Add Grade</span> 
            </BreezeButton>
        </div>
    ...
    <div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" id="gradeAddEditModal" tabindex="-1" aria-labelledby="gradeAddEditModalLabel" aria-modal="true" role="dialog">
        ...
        <div class="modal-dialog modal-lg relative w-auto pointer-events-none">
            <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-dark-blue bg-clip-padding rounded-md outline-none text-current ">
                <div class="modal-header flex flex-shrink-0 items-center justify-between p-4 border-b border-gray-200 rounded-t-md">
                    <h5 class="text-xl font-medium leading-normal text-white" id="gradeAddEditModalLabel">
                     {{ action }} Grade
                    </h5>
                    <button type="button"
                ...
                <div class="modal-body relative p-4">
                    <form id="mainGradeForm" ref="mainGradeForm" method="post" v-on:submit.prevent="saveForm">
                        <div class="flex flex-wrap -mx-2">
                            <div class="my-2 px-2 w-full sm:w-full md:w-full lg:w-1/2 xl:w-1/2">
                                <o-field label="Name *"  :variant="errors.name ?'danger':''" :message="errors.name?errors.name.toString():''">
                                    <o-input v-model.trim.lazy="grade.name"></o-input>
                                </o-field>
                    
                                <o-field label="Description" :variant="errors.description ? 'danger':''" :message="errors.description?errors.description.toString():''">
                                    <o-input maxlength="2000" type="textarea" v-model.trim.lazy="grade.description"></o-input>
                                </o-field>
                            </div>

                            <div class="my-2 px-2 w-full sm:w-full md:w-full lg:w-1/2 xl:w-1/2">

                                <o-field class="file" label="Grade logo" ref="logoUpload" :variant="errors.image_url ? 'danger':''" :message="errors.image_url?errors.image_url.toString():''">
                                    <o-upload v-model="grade.image_url" accept="image/*">
                                        <o-button tag="a" variant="primary">
                                            <span class="mdi mdi-upload">Upload</span>
                                        </o-button>
                                    </o-upload>
                                    <span class="file-name" v-if="grade.image_url">
                                        {{ grade.image_url.name }}
                                    </span>
                                </o-field>

                                <o-field label="Delete Grade Logo?" v-if="grade.main_img != null">
                                    <BreezeCheckbox v-model:checked="grade.delete_grade_logo" color="secondary" />
                                </o-field>

                                <o-field label="Color" >
                                    <BreezeColorPicker v-model="grade.color"></BreezeColorPicker>
                                </o-field>

                                <o-field class="py-4">
                                    <o-switch v-model="grade.active" true-value="1" false-value="0">
                                        Active
                                    </o-switch>
                                </o-field>
                        ...
                        <div class="flex flex-wrap -mx-2">
                            <div class="my-2 px-2 w-full sm:w-full md:w-full lg:w-1/2 xl:w-1/2">
                                <o-field label="Add some items">
                                    <o-inputitems v-model="grade.tags" icon="tag" placeholder="Add an item" aria-close-label="Delete this item"> </o-inputitems>
                                </o-field>
                            </div>
                        </div>
    ...
    <BreezeViewModel :model="'Grade'" :isLoading="isLoadingView" :modelData="grade" ref="viewModelRef"></BreezeViewModel>
    <BreezeConfirmDialog title="Delete Grade" content="Are you sure you want to delete Grade?" ref="deletemodelRef">
