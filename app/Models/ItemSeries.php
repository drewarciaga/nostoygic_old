...
class ItemSeries extends MyBaseModel
{
    ...
    public function uploadLogo($request){
        $status = "success";
        $folder = 'item_images/series';

        //delete existing image first
        $this->deleteImage($this->image_url, $this->thumbnail_url);

        $url = $this->uploadImage($request, 'image_url', $folder, 800, 'series_img');
        if(!empty($url)){
            if($url == 'error'){
                $status = "error";
            }else{
                ...
                if(!empty($this->image_url)){
                    $thumbUrl = $this->createThumbnail($request, 'image_url', $folder, 300, 'series_img');
                    if($thumbUrl == 'error'){
                        $status = "error";
                    }else{
